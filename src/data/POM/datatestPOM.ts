export class datatest{
    public static validFirstName = "INTERNAL TESTING ONLY";
    public static validLastName = "PLEASE IGNORE";
    public static validEmail = "kipen56854@pashter.com";
    public static comment ="Our codeless automation tool allows you to rapidly";
    //page thank you
    public static textheader= "Thank you!";
   //text content thank you
   public static content="We have sent your email to a member of our sales team, and you can expect a reply soon.";
   //page Home page
   public static header= "Cut down the time to find unique locators by half*";
}